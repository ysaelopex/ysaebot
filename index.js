...
client.on(
    ...
    async member => {
        ...
        try {
            ...
            // ======================================================
            // O ID do cargo está configurado em CARGO_AUTOROLE.
            // Busca o cargo no servidor e entrega ao novo membro.
            const cargoAutomatico =
                await guild.roles.fetch(CARGO_AUTOROLE).catch(() => null);

            if (!cargoAutomatico) {
                console.log(
                    `❌ Cargo automático ${CARGO_AUTOROLE} não foi encontrado no servidor ${guild.id}.`
                );
            } else if (
                guild.members.me &&
                cargoAutomatico.position >= guild.members.me.roles.highest.position
            ) {
                console.log(
                    `❌ Não foi possível entregar o cargo automático ${CARGO_AUTOROLE}: o cargo do bot precisa estar acima dele na hierarquia.`
                );
            } else if (!member.roles.cache.has(CARGO_AUTOROLE)) {
                await member.roles.add(
                    cargoAutomatico,
                    'Cargo automático ao entrar no servidor'
                );
                console.log(
                    `✅ Cargo automático ${CARGO_AUTOROLE} entregue para ${member.user.tag}.`
                );
            }
